...
This is for teaching Codex how to use 3dsmax.

## Flags
- [x] **learn-from-mistakes** — When enabled, Codex MUST update `skills/3dsmax-mcp-dev/SKILL.md` whenever it encounters a bug, unexpected behavior, or discovers a MAXScript/3ds Max pitfall during a session. This keeps the skill file growing with real-world lessons so future sessions (and anyone who clones this repo) benefit immediately.

### How learn-from-mistakes works
1. Codex hits an error or unexpected result
2. Codex fixes the issue
3. Codex appends the lesson to the relevant section in `skills/3dsmax-mcp-dev/SKILL.md`
...
- `.Codex/skills/` is generated by `scripts/build_skill.py` and gitignored
